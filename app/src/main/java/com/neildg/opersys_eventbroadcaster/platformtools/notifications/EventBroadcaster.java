...
public class EventBroadcaster {
	...
	/*
	 * Attaches an activity so any events are queued into the UI thread. Not doing so when attempting to broadcast from a thread  will cause a
	...
	public void postNotification(final String notificationString, final Parameters parameters) {
		if(this.activity != null) {
			this.activity.runOnUiThread(new Runnable() {
				@Override
				public void run() {
					actualNotify(notificationString, parameters);
				}
			});
		}
		else {
	...
	public void postNotification(final String notificationString) {
		if(this.activity != null) {
			this.activity.runOnUiThread(new Runnable() {
				@Override
				public void run() {
					actualNotify(notificationString);
				}
			});
		}
		else {
